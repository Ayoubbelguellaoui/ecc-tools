...
{
  // PrimeTime defines all_clocks as get_clocks -quiet *.  Use the same
  // query path as get_clocks so ordering and object matching stay consistent.
  setResult(queryObjects(STADM.getDatabase(), {"*"}, QueryObjectType::kClock));
  return 1;
}
...
unsigned TclGetFullName::exec()
{
  ...
  try {
    setResult(resolveFullNames(STADM.getDatabase(), object->getStringVal()));
