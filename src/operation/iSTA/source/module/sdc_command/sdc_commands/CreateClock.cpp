...
std::string inferredClockName(const std::vector<std::string>& source_list)
{
...
unsigned TclCreateClock::exec()
{
  ...
  if (object_option->is_set_val()) {
    try {
      source_list = resolveClockSources(database, object_option->getStringList());
  ...
  const std::string clock_name = name_option->is_set_val() ? std::string(name_option->getStringVal()) : inferredClockName(source_list);
  if (clock_name.empty()) {
