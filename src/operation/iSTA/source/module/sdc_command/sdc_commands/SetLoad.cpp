...
std::vector<AnalysisType> selectedAnalysisTypes(SdcTclCmd& command)
{
...
std::vector<TransType> selectedTransTypes(SdcTclCmd& command)
{
...
LoadTargets resolveLoadTargets(Database& database, const std::vector<std::string>& objects)
{
  LoadTargets targets;
  for (const std::string& object : objects) {
    const std::vector<std::string> ports = queryObjects(database, {object}, QueryObjectType::kPort);
    const std::vector<std::string> nets = queryObjects(database, {object}, QueryObjectType::kNet);
...
unsigned TclSetLoad::exec()
{
  ...
  LoadTargets targets;
  try {
    targets = resolveLoadTargets(database, object_option->getStringList());
  ...
  // PrimeTime and OpenSTA both treat "-pin_load -wire_load" the same as the
  // default: a port pin load. Wire load is selected only when it is the sole
  // load-kind option.
  ...
  for (AnalysisType analysis_type : selectedAnalysisTypes(*this)) {
    for (TransType trans_type : selectedTransTypes(*this)) {
