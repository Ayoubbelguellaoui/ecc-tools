...
std::set<std::string> selectedClockSet(Database& database, SdcTclCmd& command, std::initializer_list<const char*> options)
{
  int option_count = 0;
  ...
  return resolveClockObjects(database, command.getOptionOrArg(selected)->getStringList());
}

TransType selectedTransition(SdcTclCmd& command, const char* rise_option, const char* fall_option)
{
...
unsigned TclSetClockUncertainty::exec()
{
  ...
  if (hasPairSelector(*this)) {
    try {
      ...
      pair_uncertainty.set_from_clocks(selectedClockSet(database, *this, {"-from", "-rise_from", "-fall_from"}));
      pair_uncertainty.set_to_clocks(selectedClockSet(database, *this, {"-to", "-rise_to", "-fall_to"}));
      pair_uncertainty.set_from_trans_type(selectedTransition(*this, "-rise_from", "-fall_from"));
      TransType to_trans_type = selectedTransition(*this, "-rise_to", "-fall_to");
      if (to_trans_type == TransType::kNone && getOptionOrArg("-to")->is_set_val()) {
        to_trans_type = selectedTransition(*this, "-rise", "-fall");
      }
