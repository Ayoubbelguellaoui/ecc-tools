...
TclGetPorts::TclGetPorts(const char* cmd_name, ClientData client_data) : SdcTclCmd(cmd_name, client_data)
{
  ...
  addQueryOptions(*this, false, true, true);
}
...
unsigned TclGetPorts::exec()
{
  ...
  std::vector<std::string> result = queryObjects(STADM.getDatabase(), queryPatterns(patterns, options.regexp), QueryObjectType::kPort, options);
