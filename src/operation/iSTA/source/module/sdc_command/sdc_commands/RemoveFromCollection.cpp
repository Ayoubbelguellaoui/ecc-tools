...
unsigned TclRemoveFromCollection::exec()
{
  ...
  const std::vector<std::string> collection = queryPatterns(collection_option->getStringVal(), false);
  ...
  if (homogeneous && type && *type != QueryObjectType::kAny) {
    const std::vector<std::string> matches = queryObjects(database, objects, *type);
