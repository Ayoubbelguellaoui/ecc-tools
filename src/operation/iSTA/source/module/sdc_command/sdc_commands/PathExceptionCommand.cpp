...
std::vector<TimingException> expandSelectorObjects(const TimingException& exception)
{
  std::vector<TimingException> expanded{exception};
  const auto expand_set = [&expanded](const std::set<std::string>& objects, auto setter) {
    if (objects.size() <= 1) {
      return;
    }
  ...
  expand_set(exception.get_from_objects(), [](TimingException& item, const std::set<std::string>& objects) { item.set_from_objects(objects); });
  expand_set(exception.get_to_objects(), [](TimingException& item, const std::set<std::string>& objects) { item.set_to_objects(objects); });
  for (std::size_t through_index = 0; through_index < exception.get_through_list().size(); ++through_index) {
    ...
    expand_set(objects, [through_index](TimingException& item, const std::set<std::string>& selected) {
...
std::vector<TimingException> subtractDimensions(const TimingException& existing, const TimingException& filter)
{
...
TimingException TclPathException::parsePathSelector(Database& database, bool require_selector)
{
  ...
  for (const auto& [option, value] : getOptionValueList()) {
    ...
    through.set_objects(resolveExceptionObjects(database, queryPatterns(value, false)));
...
void resetPathExceptions(Database& database, const TimingException& filter)
{
  ...
  for (const TimingException& existing : database.get_timing_constraint().get_path_exception_list()) {
    ...
    for (const TimingException& expansion : expandSelectorObjects(existing)) {
      if (!selectorIntersects(filter, expansion)) {
        result.push_back(expansion);
        continue;
      }
      std::vector<TimingException> remaining = subtractDimensions(expansion, filter);
